  ...
  async generateInvestigationReport(
    ...
    if (typeof diagnosis === 'string') {
      try {
        ...
        throw new Error('diagnosis must be a valid JSON object');
      }
    }
    ...
    if (typeof impact === 'string') {
      try {
        ...
        throw new Error('impact must be a valid JSON object');
      }
    }
    ...
    const evidence = Array.isArray(diagnosisDetails.evidence)
      ? diagnosisDetails.evidence
      : [];

    const recommendations = Array.isArray(diagnosisDetails.recommendations)
